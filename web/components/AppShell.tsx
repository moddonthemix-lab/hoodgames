...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      {/* Screens render live countdowns/cooldowns from Date.now(); only mount them client-side to
          avoid SSR/hydration text mismatches (see useMounted). */}
      <main className="flex-1 overflow-y-auto px-4 py-4">{mounted ? children : null}</main>
      <BottomNav />
    </div>
  );
}
